import { BOMB_MAX_PLAYERS, BOMB_MIN_PLAYERS, type ServerMessage } from '../shared/protocol';
import { startBomb, type Ctx, type Bomb } from './passTheBomb';

/**
 * Pass the Bomb's start guard. Same harness shape as spill.test.ts — a fake `Ctx` with a
 * clock we control (docs/testing.md §1.1).
 *
 * **This covers the guard only.** The rest of `passTheBomb.ts` — the pairing window, the
