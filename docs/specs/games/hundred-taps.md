...
the puzzle. Tap 1, then 2, then 3, scanning the whole grid each time to find
the next one, wherever it landed. Tap the wrong circle — a number out of
order, or one already cleared — and you fall back to your last checkpoint of
ten (§2.2), the same forgiving rule Tap Tap Music built for its own version
of this mistake.
...
2. All 100 numbers are printed on their cells and visible immediately.
   ...
   finding *where* it is.
3. Tap the cell showing the next number in order. It goes **gone**
   (hollowed out), your cleared count advances by one, and a note plays a
   little higher than the last (§5b).
4. **Tap any other visible number — one already cleared, or one still ahead
   — and you fall back to your last checkpoint of ten (§2.2).** Exactly Tap
   Tap Music's rule, reused rather than reinvented. Every cell undone
   returns to being a valid next target once its turn comes back around.
   Nobody else's board is touched.
...
Taps' board hides nothing: every number is printed and visible, so the
player already knows exactly what to look for. That removes the need for a
window entirely. **Only the single cell showing `cleared + 1` is ever a
correct tap** — everything else, cleared or not, is a miss. The shared,
server-dealt `order` array is reused unchanged from Tap Tap Music
(`shared/protocol.ts`, `taptap`/`tap-tap-music.md` §2.1) — only its
*meaning* changes, from "the hidden sequence to reveal" to "which grid cell
holds which printed number."
...
- **Round**: a timeline strip above a 10×10 grid of numbered circles,
  ...
  cell always shows its own printed number. Two visual states, never colour
  alone: **live** (full opacity, number visible, gradient fill) and **gone**
  (hollow, faded, number still legible, still tappable — spec §7, same
  "tapping a gone cell is exactly as wrong as any other wrong cell" rule Tap
  Tap Music's board already establishes). The status bar shows cells
  remaining and the running clock in `SS.CC`.
...
| A tap lands on a cell that is already gone, or any number other than the exact next one | A miss — rewinds to the last checkpoint (§2.2) |
...
- Cells never rely on colour alone for their state: live vs. gone differ by
  opacity and outline style, and — unlike Tap Tap Music — every cell always
  shows its own printed number regardless of state, so the pink-to-violet
...
- **Is a static, all-visible board actually harder or easier than Tap Tap
  Music's window?** Untested against real thumbs — a full-grid scan-and-find
  task (a "Schulte table") is a different kind of difficulty than following
  a moving window, and nothing here says which is more fun at speed.
