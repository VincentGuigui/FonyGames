...
2. **Does the dim terrain memory make it too easy?** Still open, and still the
   ...
   do it.
3. **The guarantee is verified, not trusted.** The roller builds a map to have
...
4. **The unlit board is invisible, and that is intentional but untested.** An
...
5. **Trees are new.** The spec's map has traps and monsters; the build adds
