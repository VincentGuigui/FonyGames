...
| D3 | Realtime transport | Self-hosted Node + `ws` · WebRTC data channels (peer-to-peer) · managed service (PartyKit / Cloudflare Durable Objects / Supabase Realtime) | **Node + `ws`** — smallest thing that works, swap later behind `core/room` |
| D4 | Hosting | GitHub Pages + a small VPS · Cloudflare Pages + Workers · Vercel · self-hosted | Needs the maintainer's constraints (cost, domain, account) |
