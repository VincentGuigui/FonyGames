...
| M5d | **Cat and Mouse** | [Cat and Mouse](specs/games/cat-and-mouse.md) specced, including two host-selected drag modes and a deliberate no-fallback decision (§12). **Not built**: it is the first game needing streamed positions, and the scribbling/cooldown balance in `direct` is unproven — that is what wants a decision before code | 📝 specced, ⏳ awaiting go-ahead |
