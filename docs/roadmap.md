...
| 2026-08-03 | **Tap Duel measures reaction *and* reach.** An archer's target appears at a random spot on the signal and only a tap on it counts. Reaction alone was thin — with a thumb already on the glass there is nothing to do but twitch. The **server** picks the position so it is identical on every screen; a per-client one would decide the round by luck. `tap` still carries no coordinates: the client has to know where the target is to draw it, so a position field could not be validated and would buy nothing | [specs/games/tap-duel.md](specs/games/tap-duel.md) §4, §8 |
