...
**Six cards are exceptions to "cards are pure vector":** Tap Fighter, Aliens
love cows, Random Game, Gravity Shooter, Asteroid Race and Tilt Race. Every
other card redraws its sprite's shape as fresh paths (this section's Goat
Siege/Sling Puck/Spill examples); Tap Fighter's `card.svg` instead embeds
base64 crops of the actual `fighter1.png`/`fighter2.png` frames, Aliens love
cows' embeds crops of its own `ufo.png`, `barn.png` and `cow.png` (the cone,
the stars and the ground stay fresh vector paths around them), Random Game's
embeds a downscaled crop of `art/src/dice.png` over a mosaic of eight other
games' own card art (the mosaic tiles stay fresh — they are those games' real
`card.svg` markup, not a redraw), Gravity Shooter's embeds its own two ship
sprites, two planet sprites and its missile, Asteroid Race's embeds one frame
cropped straight out of its own `ship.png`, and Tilt Race's embeds a crop of
its own `car.png` plus a small tile of `road.jpg` — because a hand-drawn
approximation of a detailed authored photo or pixel-art subject reads worse
than the real thing at this size (#41).

**Five of the six are generated** rather than hand-assembled, all gated by a
committed content-hash manifest (`art/.card-manifest.json`) the same way
`og.mjs` guards its own derived PNGs, all regenerated by an `art:` script, and
all failing `npm test` when the committed file is stale:

- `aliens-love-cows/generate-card.mjs` crops `ufo.png`, `barn.png` and
  `cow.png` straight out of this game's own `art/` (`npm run art:abduct-card`)
  — the last of the three subjects to move off a hand-drawn approximation
  once the round screen itself got a real UFO sprite. A cropped photo cannot
  carry a stroke, so the card's own accent (required by `cards.test.mjs`) is
  a thin ring at the ship's own rim instead of the old dome outline's colour.
  The cone, the stars and the ground stay hand-drawn vector.
- `random-game/generate-card.mjs` extracts the eight mosaic tiles and
...
- `asteroid-race/generate-card.mjs` crops one fixed cell straight out of
  `ship.png` (`npm run art:asteroid-card`) — a card has no steer to pick a
  frame with, so the cell is named outright rather than derived. Everything
  else on the card (the tube, the rocks, the crosshair) stays hand-drawn
  vector, the same split Aliens Love Cows makes between its sprited subjects
  and its fresh-vector surroundings.
- `tilt-race/generate-card.mjs` (#41) crops `car.png` the same way
  `asteroid-race`'s crops `ship.png` (`npm run art:tilt-card`), but the road
  is a **pattern**, not a placed image: a small square cropped from the centre
  of `road.jpg`, downscaled hard, and referenced as `stroke="url(#road)"` on
  the same kerb path the hand-drawn version already stroked — the SVG
  `<pattern>` tiles in its own user-coordinate space exactly the way
  `TrackCanvas.tsx`'s `ctx.createPattern(roadImg, 'repeat')` tiles in world
  space, so one tiny embedded tile covers however much of the bend is drawn.
  The car's own rotation is the hand-drawn dart's unchanged `rotate(130)`,
  reusable because `car.png` is authored nose-up at the dart's own local
  `(0, -h/2)`.
...
cap the moment its card became generated. It left the mosaic and Asteroid Race
took its place; Asteroid Race then left the same way once its own ship became
a generated crop, and UFO Hunt took its place — which is the same reason Tap
Fighter, Aliens Love Cows and Tilt Race were never in it.

Two things make any of the six safe rather than a shortcut: a bare `<image href="./fighter1.png">` would 404 once Vite hashes the
...
50–120 KB each, base64 alone would blow the budget several times over. Tilt
Race's road tile pays that budget only once regardless of how many times the
pattern repeats, which is what keeps a whole extra texture (`road.jpg`,
~120 KB raw) inside the same 40 KB cap the other five squeeze one sprite into.
