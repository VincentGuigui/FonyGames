...
stay fresh vector paths around them), Random Game's embeds a downscaled crop
of `art/src/dice.png` over a mosaic of eight other games' own card art (the
mosaic tiles stay fresh — they are those games' real `card.svg` markup, not a
redraw), Gravity Shooter's embeds its own two ship sprites, two planet sprites
and its missile, and Asteroid Race's embeds one frame cropped straight out of
its own `ship.png` — because a hand-drawn approximation of a detailed
authored pixel-art subject reads worse than the real thing at this size.

**Three of the five are generated** rather than hand-assembled, all gated by a
...
Fighter and Aliens Love Cows were never in it.

Two things make any of the five safe rather than a shortcut: a bare `<image href="./fighter1.png">` would 404 once Vite hashes the
