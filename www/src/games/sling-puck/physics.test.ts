...
function walls(): void {
  ...
  const bottom = [puck({ x: 0.5, y: 0.7, vy: 0.8 })];
  run(bottom, 0.5);
  check('the bottom wall turns it around', (bottom[0]?.vy ?? 0) < 0, bottom[0]);

  // The top wall bounces everywhere except the gap — the whole point.
  // Long enough to reach the wall and come back: friction eats the 0.8 start
  // speed, so the trip up takes ~0.75 s, not the ~0.55 s it would take unslowed.
  const solidTop = [puck({ x: GAP_LEFT - 0.08, y: 0.5, vy: -0.8 })];
  const out = run(solidTop, 1);
...
function gap(): void {
  ...
  check('the crossing reports where it left', (out[0]?.x ?? -1) > GAP_LEFT && (out[0]!.x) < GAP_RIGHT, out[0]);
  ...
  check('just inside the gap edge crosses', run(inside, 1).length === 1);
  const outside = [puck({ x: GAP_LEFT - 0.01, y: 0.5, vy: -0.8 })];
  check('just outside it does not', run(outside, 1).length === 0);
}
...
function tunnelling(): void {
  ...
  const p = [puck({ x: 0.12, y: 0.95, vy: -fastest })];
  const out = run(p, 3);
  check('it never leaves through a wall', out.length === 0, out);
  check('it is still on the board', p.length === 1);
  check('and still inside it', (p[0]?.y ?? -1) >= PUCK_RADIUS - 1e-6 && (p[0]!.y) <= 1, p[0]);

  // Same speed straight across, to catch the side walls too.
  const across = [puck({ x: 0.5, y: 0.5, vx: fastest })];
  run(across, 3);
  check('a fast sideways puck stays inside', (across[0]?.x ?? -1) <= 1 - PUCK_RADIUS + 1e-6, across[0]?.x);
}

function friction(): void {
  console.log('\nfriction brings it to a dead stop');

  const p = [puck({ x: 0.5, y: 0.5, vx: 0.3, vy: 0.1 })];
  run(p, 6);
  check('it stops completely', p[0]?.vx === 0 && p[0]?.vy === 0, p[0]);

  // A bounce must lose energy, or a puck would rattle forever.
  const b = [puck({ x: 0.5, y: 0.6, vy: 0.6 })];
  step(b, 1 / 60);
  const before = 0.6;
  run(b, 0.4);
  check('a bounce loses energy', Math.abs(b[0]?.vy ?? 0) < before * RESTITUTION + 0.01, b[0]);
}
...
function sling(): void {
  ...
  // Further back is faster, up to the cap.
  const soft = slingVelocity(mid, POST_LEFT.y + MAX_PULL * 0.2);
  const hard = slingVelocity(mid, POST_LEFT.y + MAX_PULL);
  const speed = (v: { vx: number; vy: number }) => Math.sqrt(v.vx * v.vx + v.vy * v.vy);
  check('a longer pull is faster', speed(hard) > speed(soft), { soft, hard });
  check('no pull is no launch', speed(slingVelocity(mid, POST_LEFT.y)) < 1e-9);

  const capped = slingVelocity(mid, POST_LEFT.y + 5);
  check('speed is capped', speed(capped) <= MAX_SPEED + 1e-9, speed(capped));

  // A pull cannot be dragged forwards past the band or off the board.
  const c = clampPull(-3, 0.1);
  check('a pull cannot go off the side', c.x >= PUCK_RADIUS, c);
  check('nor in front of the band', c.y >= POST_LEFT.y, c);
}
...
function launchLandsIt(): void {
  ...
  const out = run(p, 3);
  check('a good shot goes through', out.length === 1, { out, v });
}
...
function opening(): void {
  ...
  check('all inside the board', pucks.every((p) => p.x >= PUCK_RADIUS && p.x <= 1 - PUCK_RADIUS));
...
function collisions(): void {
  ...
  // Head-on into a stationary puck: equal masses, so the mover should stop
  // dead-ish and the target should take the speed.
  const a = puck({ id: 1, x: 0.3, y: 0.5, vx: 0.8 });
  const b = puck({ id: 2, x: 0.3 + PUCK_RADIUS * 2 + 0.005, y: 0.5 });
  const pair = [a, b];
  run(pair, 0.3);
  ...
  const shooter = puck({ id: 3, x: 0.5, y: 0.6, vy: -1.2 });
  const sitting = puck({ id: 4, x: 0.5, y: 0.6 - PUCK_RADIUS * 2 - 0.01 });
  const out = run([shooter, sitting], 3);
  check('a puck can knock another through', out.length >= 1, out);

  // And they must never end up stacked on the same spot.
  const heap = [
    puck({ id: 5, x: 0.5, y: 0.5 }),
    puck({ id: 6, x: 0.5, y: 0.5 + 0.001 }),
    puck({ id: 7, x: 0.5 + 0.001, y: 0.5 }),
  ];
  run(heap, 1);
...
for (const t of [walls, gap, tunnelling, friction, sling, launchLandsIt, opening, collisions]) {
  t();
}
