...
function Inner({ code, game }: { code: string; game: GameCard }): JSX.Element {
  const t = useT(); const text = useGameText(); const solo = soloTesting(); const [, redraw] = useState(0); const ref = useRef(new TttGame()); const model = ref.current;
  ...
  if (state?.phase === 'over') {
    const winner = state.winner; return <GameOverScreen accent={game.accent} title={game.title} concept={game.concept} rules={game.rules} slug={game.slug} winner={winner} rows={players.map((p) => ({ id: p.id, name: p.name, avatar: p.avatar, value: symbol(p.id) }))} me={me} onAgain={() => client?.send({ t: 'start', d: { mode: 'tttt', solo, symbols: { x: x ?? '', o: o ?? '', chooser: chooser ?? '' } } })} canAct={r.room.isHost} />;
  }
