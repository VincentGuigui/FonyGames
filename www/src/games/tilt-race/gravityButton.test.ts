...
function directions(): void {
  ...
  // Halfway poses land between, and the two rolls are mirror images.
  const rolledRight = downVector(-45, 45);
  const rolledLeft = downVector(45, 45);
  check('a roll one way has a rightward pull', rolledRight.x > 0.5, rolledRight);
  check('and the other a leftward one', rolledLeft.x < -0.5, rolledLeft);
