 ...
 *
 * A "variant" is one (state × hot × week × showAll) combination. They are enumerated
 * mechanically by calling `cardState()` — the same function the hub, the admin centre and
 * the Worker's rules all use — so no decision is duplicated anywhere. `hot` and `week`
 * each double the count and most of the new entries are byte-identical to their twin,
 * because only an `active`/`new` card wears a badge; that redundancy is the price of
 * `index.php` being able to ask for a key without first knowing which combinations collapse.
 *
 ...
 *
 * The reason for a soon card is the ONE runtime string, so it is rendered with a
...
export function cards() {
  ...
  for (const game of catalogue()) {
    const variants = {};
    for (const state of ['new', 'active', 'soon', 'hidden']) {
      for (const hot of [false, true]) {
        for (const week of [false, true]) {
          for (const showAll of [false, true]) {
            const flag = { state, reason: '%%REASON%%' };
            ...
            const key = state + ':' + (hot ? 1 : 0) + ':' + (week ? 1 : 0) + ':' + (showAll ? 1 : 0);
            variants[key] = view.show
              ? renderToString(h(GameCardTile, { game, flag, showAll, hot, week }))
              : '';
          }
        }
      }
...
/**
 * Every live game, alphabetical by its own (English) title — never a locale's
 * translation of it, so \`gameOfWeek()\` (shared/flags.ts) lands on the same slug
...
/**
 * Every not-yet-live game, in its curated \`registry.ts\` order — the trailing tier
 * \`HubGrid.tsx\` appends after \`hubSections()\`'s three (issue #4). \`hubSections\` only
 * ever sorts live games, so a \`soon\` card cannot be hot, spotlighted or NEW; without this
 * it would have nowhere left to render at all, having dropped out of \`weekOrder()\` above.
 */
export function soonOrder() {
  return catalogue()
...
const { shell, cards, order, weekOrder, soonOrder, gridShell } = await import(`${importUrl(BUNDLE)}?v=${count0()}`);
...
// One entry per (state : hot : week : showAll). '' means the card is not rendered.
// Spec: docs/specs/seo.md §4
return ${phpArray({ week: weekOrder(), soon: soonOrder(), grid: gridShell(), cards: variants })};
`,
);
...
foreach ($slugs as $slug) {
    // 'hidden' means not reachable at all; 'soon' is still a real page that shows a
    // greyed card, so it stays listed.
    $state = $flags[$slug]['state'] ?? 'active';
