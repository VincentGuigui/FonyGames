 ...
 *
 * A "variant" is one (availability × isNew × hot × week × showAll) combination. They are
 * enumerated mechanically by calling `cardState()` — the same function the hub, the admin
 * centre and the Worker's rules all use — so no decision is duplicated anywhere. `hot` and
 * `week` each double the count and most of the new entries are byte-identical to their
 * twin, because only an `active` card wears a badge; that redundancy is the price of
        ...
        for (const hot of [false, true]) {
          for (const week of [false, true]) {
            for (const showAll of [false, true]) {
              ...
              const key = availability + ':' + (isNew ? 1 : 0) + ':' + (hot ? 1 : 0) + ':' + (week ? 1 : 0) + ':' + (showAll ? 1 : 0);
