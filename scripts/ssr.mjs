 ...
 *
 * A "variant" is one (availability × isNew × showAll) combination. They are enumerated
 * mechanically by calling `cardState()` — the same function the hub, the admin centre and
 * the Worker's rules all use — so no decision is duplicated anywhere.
 *
    ...
    for (const availability of ['active', 'disabled', 'hidden']) {
      for (const isNew of [false, true]) {
        for (const showAll of [false, true]) {
          ...
          const key = availability + ':' + (isNew ? 1 : 0) + ':' + (showAll ? 1 : 0);
          variants[key] = view.show
            ? renderToString(h(GameCardTile, { game, flag, showAll }))
            : '';
        }
      }
    }
...
// One entry per (availability : isNew : showAll). '' means the card is not rendered.
// Spec: docs/specs/seo.md §4
...
echo Page::render(
    ...
    Page::grid($built['order'], $built['cards'], $flags, $showAll),
    $flags,
    $showAll,
);
`,
