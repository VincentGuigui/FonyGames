...
const { shell, cards, order, weekOrder, gridShell } = await import(`${importUrl(BUNDLE)}?v=${count0()}`);
...
// Spec: docs/specs/seo.md §4
return ${phpArray({ order: order(), week: weekOrder(), grid: gridShell(), cards: variants })};
`,
);
...
echo Page::render(
    ...
    Page::grid($built['order'], $built['cards'], $flags, $showAll, $plays, $built['week']),
    $flags,
    $showAll,
    $plays,
);
